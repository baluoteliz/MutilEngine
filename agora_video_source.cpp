...
namespace agora{
    namespace rtc{
        ...
        public:
			...
			virtual node_error setVideoSourceVideoEncoderConfiguration(VideoEncoderConfiguration config) override;
			...
			virtual node_error setupLocalVideo(HWND hWnd, agora::rtc::RENDER_MODE_TYPE renderMode) override;
            ...
            virtual node_error setParameters(const char* parameters) override;
        private:
            void msgThread();
        ...
        bool AgoraVideoSourceSink::initialize(IAgoraVideoSourceEventHandler *eventHandler, const char* appid)
        {
            ...
            do {
                ...
                std::string cmdname = "VideoSourceStream";
		...
		node_error AgoraVideoSourceSink::setupLocalVideo(HWND hWnd, agora::rtc::RENDER_MODE_TYPE renderMode)
		{
			std::unique_ptr<LocalVideoCmd> cmd(new LocalVideoCmd());
			cmd->renderMode = agora::rtc::RENDER_MODE_FIT;
			cmd->view = hWnd;
			return m_ipcMsg->sendMessage(AGORA_IPC_SETUP_LOCAL_VIDEO, (char*)cmd.get(), sizeof(cmd)) ? node_ok : node_generic_error;
		}
        ...
        node_error AgoraVideoSourceSink::setParameters(const char* parameters)
        {
			if (!m_initialized)
				return node_status_error;
            SetParameterCmd cmd;
            strncpy_s(cmd.parameters, parameters, MAX_PARAMETER_LEN);
           return m_ipcMsg->sendMessage(AGORA_IPC_SET_PARAMETER, (char*)&cmd, sizeof(cmd)) ? node_ok : node_generic_error;
        }
		...
		node_error AgoraVideoSourceSink::setVideoSourceVideoEncoderConfiguration(VideoEncoderConfiguration config)
		{
			if (m_initialized){
				VideoEncoderConfigurationCmd cmd(config);
				return m_ipcMsg->sendMessage(AGORA_IPC_SET_VIDEO_ENCODER_CONFIGURATION, (char*)&cmd, sizeof(cmd)) ? node_ok : node_generic_error;
			}
			return node_status_error;
		}
